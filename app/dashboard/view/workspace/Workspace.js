...
class Workspace extends Component {
      ...
      <div className='dash-workspace'>
        ...
        {workspaces.length > 0 && (
          <div className='created'>
            <div className='caption'>
              <div className='title'>
                {i18n('ws.myWorkspace')} ({workspaces.length})
              </div>
              <div className='tip'>
                {i18n('ws.wsTip', { limit: wsLimit })}
                <a
                  href='https://dev.tencent.com/help/cloud-studio/workspace-introduction'
                  target='_blank'
        ...
        {workspacesCollaborative.length > 0 && (
          <div className='collaborate'>
            <div className='caption'>
              <div>
                {i18n('ws.collaborativeWorkspace')} ({workspacesCollaborative.length})
              </div>
              <div className='tip'>{i18n('ws.collaborativeWorkspaceTip')}</div>
            </div>
            <div className='card-box'>
              {workspacesCollaborative.map(ws => (
                <Card
                  key={ws.spaceKey}
                  {...ws}
                  globalKey={globalKey}
        ...
        {workspacesInvalid.length > 0 && (
          <div className='deleted'>
            <div className='caption'>
              <div>
                {i18n('global.recentdeleted')} ({workspacesInvalid.length})
              </div>
              <div className='tip'>{i18n('ws.deletedWSTip')}</div>
            </div>
            <div className='card-box'>
              {workspacesInvalid.map(ws => (
                <Card
                  key={ws.spaceKey}
                  {...ws}
                  showMask={showMask}
  ...
  handleParse (res) {
    ...
    for (let i = 0; i < res.length; i++) {
      ...
      ws.projectName =
        item.workspaceName && item.workspaceName !== 'default'
          ? item.workspaceName
          : item.projectName
  ...
  fetchInvalidWS () {
    api.getWorkspaceInvalid().then((res) => {
      if (Array.isArray(res)) {
        ...
        notification.error({
          description: res.msg || 'Failed to fetch deleted workspaceList'
        })
      }
    })
  }
