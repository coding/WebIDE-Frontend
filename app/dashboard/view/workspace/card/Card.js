...
class Card extends Component {
  render () {
    ...
    const attr = createDate
      ? `${ownerGlobalKey}/${projectName}\n${getCreatedTime(createDate)}`
      : title
    const hasRepo = !!(repoUrl && !repoUrl.includes('codingide'))
    return (
      <Href
        invalid={invalid}
        spaceKey={spaceKey}
        ...
        <div className='inner' title={attr}>
          ...
          <div className='desc'>
            {workingStatus !== 'Invalid'
              ? getModifiedTime(lastModifiedDate)
              : getDeletedTime(deleteTime)}
          </div>
  ...
  handleRepoUrl = (event) => {
    ...
    const repoHref = `${config.devOrigin}/u/${ownerGlobalKey}/p/${repoUrl
      .split('/')
      .pop()
      .split('.')
      .join('/')}`
  ...
  handleStop = (shouldOpen) => {
    ...
    api.quitWorkspace(opendSpaceKey || spaceKey).then((res) => {
      ...
      if (res.code === 0) {
        ...
        if (shouldOpen) {
          const url =
            window === window.top ? `/ws/${spaceKey}` : `${config.studioOrigin}/ws/${spaceKey}`
          window.open(url)
        } else {
...
const Href = ({ invalid, spaceKey, canotOpen, handleMask, handleStop, children }) => {
  ...
  if (canotOpen) {
    return (
      <div className='ws-card' onClick={event => handleMask(hasWorkspaceOpendOption, event)}>
        {children}
      </div>
    )
  }
  ...
  return (
    <a className='ws-card' href={href} target='_blank' rel='noopener noreferrer'>
      {children}
    </a>
  )
}
