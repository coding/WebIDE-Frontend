...
class Initialize extends Component {
  ...
  render () {
    ...
    let loadWarningStyle = {
      backgroundImage: `url(${window.serverConfig.BACKEND_URL+"/versions/load-warning"})`,
    }

    if (config.isPlatform) {
      icon = <div className='coding-loading' style={loadingStyle}></div>
      if (state.iconState === 'warning') {
        icon = <div className='coding-warning' style={loadWarningStyle}></div>
      }
    }
    if (state.errorInfo) {
      if (config.isPlatform) {
    ...
    if (state.errorCode) {
      if (state.errorCode === -1 || state.errorCode === -2) {
        ...
        if (state.errorInfo.includes('could not create workspace, the workspace number you owned is limited to')) {
          ...
          requestInfo = (
            <div className='request-info'>
              <button className='btn btn-primary' onClick={() => window.location.href = `${window.serverConfig.URI_PREFIX}/dashboard`} >{i18n`global.returnDashboard`}</button>
            </div>
          )
