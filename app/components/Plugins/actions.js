...
export const updatePackageList = registerAction(PACKAGE_UPDATE_LIST, () => {
  api.fetchPackageList().then((result) => {
    store.list.replace(
      result.map((e) => {
...
export const togglePackage = registerAction(
  PACKAGE_TOGGLE,
...
export const fetchPackageGroup = registerAction(
  FETCH_PACKAGE_GROUP,
  (groupName, pkgs, type, data) => ({ groupName: `group_${groupName}`, pkgs, type, data }),
  ({ groupName, pkgs, type, data }) => {
    const scriptArguments = pkgs.map(pkg => ({
      pkgName: pkg.name,
      pkgVersion: pkg.version,
      target: pkg.TARGET
    }))
    return api.fetchPackageScript(scriptArguments).then((script) => {
...
export const fetchPackage = registerAction(
  FETCH_PACKAGE,
  (pkg, type, data) => ({ pkg, type, data }),
  ({ pkg, type, data }) =>
    api
      .fetchPackageScript({ pkgName: pkg.name, pkgVersion: pkg.version, target: pkg.TARGET })
      .then((script) => {
...
// 插件申明加载时机，
export const loadPackagesByType = registerAction(
  PRELOAD_REQUIRED_EXTENSION,
    ...
    return fetchPackgeListPromise.then(async (list) => {
      if (config.isLib) {
        const filterList = list.filter(
          item =>
            item.name !== 'platform' && item.name !== 'Debugger' && item.name !== 'collaboration'
        )
...
export const fetchUserPackage = registerAction(FETCH_USER_PACKAGE, pkg =>
  api
    .fetchUserPackageScript(pkg.filePath)
    .then((script) => {
      pluginScriptsStore.setItem(pkg.name, script)
      return pkg.name
    })
...
export const loadPackagesByUser = registerAction(PRELOAD_USER_EXTENSION, () => {
  ...
  return Promise.all([userPackagePromise, preDeployPackagePromise]).then(
    async ([userpackages, preDeployPackages]) => {
...
export const pluginRegister = registerAction(
  PLUGIN_REGISTER_VIEW,
...
export const startRemoteHMRServer = registerAction('plugin:mount', () => {
  ...
  devSocket.on('connect_error', () => {
    notification.error({
      description: '无法连接到插件服务，请确保已经执行 yarn start 且正常启动'
    })
  })
  ...
  devSocket.on('connect', () => {
    ...
    devSocket.on('firstsend', ({ codingPackage, script }) => {
      ...
      if (!plugins || plugins.length === 0) {
        try {
          ...
          PluginRegistry.set(key || packageUniqueName, {
            ...plugin,
            pkgId: packageUniqueName,
            info: getInfo,
            loadType: 'Required'
          })
          manager.pluginWillMount()
    ...
    registerAction('plugin:unmount', () => {
      const {
        infomation: { name, version }
      } = store.pluginDevState
...
export const pluginUnRegister = registerAction(
  PLUGIN_UNREGISTER_VIEW,
  (children, callback = '') => ({ children, callback }),
  ({ children, callback }) => {
    const childrenArray = Array.isArray(children) ? children : [children]
    childrenArray.forEach(child => {
