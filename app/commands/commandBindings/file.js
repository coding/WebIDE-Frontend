...
export function openFile (obj, callback) {
  ...
  return api
    .readFile(path, encoding || currentEncoding)
    .then((data) => {
  ...
  return function createFileAtPath (path) {
    ...
    if (content) {
      return api
        ...
        .catch((res) => {
          Modal.updateModal({
            statusMessage: res.response ? res.response.data.msg : res.message
          }).then(createFileAtPath)
        })
    }
    return (
      api
...
const fileCommands = {
  ...
  'file:open_exist_file': (c) => {
    // 在所有 tabgroup 中优先打开已有的 tab
    if (typeof c.data === 'string') {
  ...
  'file:save': (c) => {
    ...
    if (!activeTab.file) {
      ...
      api.writeFile(activeTab.file.path, content).then((res) => {
  ...
  'file:save_monaco': (context) => {
    ...
    const activeTab = EditorTabState.activeTab
    if (activeTab.file) {
      api.writeFile(activeTab.file.path, data).then((res) => {
        FileStore.updateFile({
          path: activeTab.file.path,
          isSynced: true,
          lastModified: res.lastModified
  ...
  'file:rename': (c) => {
    ...
    const existingTabs = TabStore.findTab(tab => tab.file && tab.file.path.startsWith(oldPath))

    const moveFile = (from, to, force) => {
      api
        .moveFile(from, to, force)
  ...
  'file:delete': async (c) => {
    ...
    if (confirmed) {
      api
        .deleteFile(c.context.path)
        .then(() =>
          notification.success({
            description: i18n`file.deleteNotifySuccess`
          })
        )
        .catch(err =>
          notification.error({
            description: i18n`file.deleteNotifyFailed${err.msg}`
          })
        )
    }
  ...
  'file:open_welcome': (c) => {
    // const activeTabGroup = TabStore.getState().activeTabGroup
    const existingTabs = TabStore.findTab(tab => tab.type === 'welcome')
    if (existingTabs.length) {
      const existingTab = existingTabs[0]
      existingTab.activate()
    } else {
  ...
  'file:open_changelog': () => {
    const existingTabs = TabStore.findTab(tab => tab.type === 'changelog')
    if (existingTabs.length) {
      const existingTab = existingTabs[0]
      existingTab.activate()
    } else {
  ...
  'file:add_ignore': (c) => {
    ...
    if (ignoreExists) {
      api.readFile(ignorePath).then((ignoreFile) => {
        ...
        !ignoreContent.split('\n').some(item => patchTxt.startsWith(item))
          ? api
              ...
              .then((res) => {
                ...
                notification.success({
                  description: i18n`file.updateIgnoreSuccess`
                })
                openFile({ path: ignorePath })
              })
              .catch((err) => {
                notification.error({
                  description: i18n`file.updateIgnoreFailed${{ err: err.msg }}`
                })
              })
          : notification.error({
            description: i18n`file.updateIgnoreTip${{ path }}`
          })
      })
    } else {
      api
        .createFile(ignorePath, patchTxt)
        .then((res) => {
          if (res.msg) {
            throw new Error(res.msg)
          }
